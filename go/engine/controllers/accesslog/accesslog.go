...
func persist(e Entry) {
	...
	if _, err := db.InsertRow("access_log", row); err != nil {
		// Surface the cause (missing table/column, type mismatch) instead of
		// silently dropping every request.
		logf("accesslog: insert failed: %v", err)
	}
}
...
// logf reports access-log subsystem problems (bad schema, DB errors) without
